...
{
    public class NutritionQueryService : INutritionQueryService
    {
        private readonly IDbConnection _db;

        public NutritionQueryService(IDbConnection db) => _db = db;

        public async Task<DailyNutritionSummaryResponse> GetDailySummaryAsync(
            int userId, DateOnly date)
        {
            ...
            var nutrients = rows.Select(r => new NutrientTotalResponse(
